...
import { CompanyProfile, InvoiceItem } from '@/types';
...
export const InvoicePreview: React.FC<InvoicePreviewProps> = ({ data, company }) => {
  ...
  if (!company) {
    return (
      <div className="bg-white shadow-xl p-12 rounded-lg text-slate-400 italic">
  ...
  return (
    <div className="relative bg-white shadow-2xl print:!shadow-none mx-auto p-12 print:p-12 w-[210mm] min-h-[297mm] overflow-hidden font-sans text-slate-800 text-sm invoice-container">
      ...
      <div
        className="top-0 left-0 absolute w-2 h-full print:no-print"
        ...
        <div className="flex flex-col justify-start items-start text-start">
          {company.logo_url ? (
            <img
              ...
              alt="Logo"
              className={`self-start rounded-lg w-64 ${
                company.name.toString().toLowerCase() === 'the nice developer' ? 'max-h-20' : 'mb-4 max-h-15'
              } object-contain object-left`}
            />
          ) : (
            <h2
              className="font-black text-slate-900 text-2xl leading-tight"
              style={{ color: company.color }}
            >
              {company.name || 'Your Company Name'}
            </h2>
          )}
          <p className="max-w-[220px] font-medium text-[10px] text-slate-500 italic leading-relaxed">
            {company.address}
          </p>
          <div className="flex flex-col items-start gap-1 mt-4 font-medium text-[10px] text-slate-600">
            {company.phone && (
              <span className="flex items-center gap-2 text-xs leading-relaxed">
                <i className="text-slate-500 fa-solid fa-phone"></i> {company.phone}
              </span>
            )}
            {company.email && (
              <span className="flex items-center gap-2 text-xs leading-relaxed">
                <i className="text-slate-500 fa-solid fa-envelope"></i> {company.email}
              </span>
            )}
            {company.website && (
              <span className="flex items-center gap-2 text-xs leading-relaxed">
                <i className="text-slate-500 fa-solid fa-globe"></i> {company.website}
              </span>
            )}
          </div>
        </div>
        <div className="flex flex-col items-end text-end">
          <h1 className="mb-4 font-black text-6xl tracking-tighter" style={{ color: company.color }}>
            INVOICE
          </h1>
      ...
      <div className="gap-12 grid grid-cols-2 mb-6">
        <div>
          ...
          <div className="mb-2 font-black text-slate-900 text-2xl leading-tight">
            {data.client.name || 'Recipient'}
          </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-slate-900 border-b-2">
              <th className="py-4 font-black text-[9px] text-slate-900 text-left uppercase tracking-[0.3em]">
                Description
              </th>
              <th className="py-4 font-black text-[9px] text-slate-900 text-center uppercase tracking-[0.3em]">
                Qty
              </th>
              <th className="py-4 font-black text-[9px] text-slate-900 text-right uppercase tracking-[0.3em]">
                Rate
              </th>
              <th className="py-4 font-black text-[9px] text-slate-900 text-right uppercase tracking-[0.3em]">
                Total
            ...
            {data.items.length > 0 ? (
              data.items.map((item) => (
                <tr key={item.id}>
                  ...
                  <td className="py-6 font-black text-slate-600 text-sm text-center align-middle">
                    {item.quantity}
                  </td>
                  <td className="py-6 font-black text-slate-600 text-sm text-right align-middle">
                    {data.currency}
      ...
      <div className="items-start gap-8 grid grid-cols-2">
        ...
        {data.notes && (
          <div>
            <div className="bg-slate-50 p-8 rounded-lg">
              <h3 className="mb-2 font-black text-[9px] text-indigo-500 italic uppercase tracking-[0.3em]">
                Official Notes
              </h3>
            ...
            {data.status === 'paid' && (
              <div className="flex justify-center items-center pt-8 h-full">
                <div className="bg-green-50 mt-4 px-4 py-2 border-4 border-green-600 rounded-lg w-max font-serif font-black text-green-600 text-4xl uppercase tracking-widest -rotate-[5deg]">
                  Paid
                </div>
              </div>
            )}
        ...
        {/* Summary Section */}
        <div className="flex justify-end mb-16 ml-auto">
          <div className="space-y-4 w-72">
            ...
            {data.paid_amount > 0 && (
              <div className="flex justify-between items-center pt-4">
                ...
                <span className="font-black text-slate-900 text-2xl leading-none">
                  {data.currency}
                  {(total - data.paid_amount).toLocaleString()}
                </span>
              </div>
            )}
          </div>
