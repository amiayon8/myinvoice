...
export default function SQLBrowser() {
  ...
  const [tabs, setTabs] = useState<QueryTab[]>([
    { id: '1', title: 'Query 1', sql: 'SELECT * FROM studentinfo_omr LIMIT 50;' }
  ]);
  ...
  const [successInfo, setSuccessInfo] = useState<{ affected_rows?: number; insert_id?: number } | null>(null);

  // Results state
  const [columns, setColumns] = useState<string[]>([]);
  const [rows, setRows] = useState<any[]>([]);
  const [activePaneTab, setActivePaneTab] = useState<'results' | 'console' | 'history'>('results');
  ...
  const fetchTableColumns = async (tableName: string) => {
    ...
    setTables(prev => prev.map(t => t.name === tableName ? { ...t, loading: true } : t));
  ...
  const handleTableToggle = (tableName: string) => {
    ...
    if (table.columns) {
      ...
      setTables(prev => prev.map(t => t.name === tableName ? { ...t, isOpen: !t.isOpen } : t));
    } else {
      // Fetch columns first
    ...
    try {
      ...
      if (!res.ok || !data.success) {
        ...
        if (data.data && Array.isArray(data.data)) {
          ...
          setSuccessInfo(null);
          addHistoryItem(sqlToRun, 'success', durationMs, `Query returned ${data.row_count || 0} rows.`);
        } else {
  ...
  const addHistoryItem = (sql: string, status: 'success' | 'error', time: number, msg: string) => {
  ...
  const formatSQL = () => {
    const keywords = [
      ...
      'set', 'delete', 'limit', 'join', 'left', 'right', 'inner', 'outer', 'on', 
      'group', 'by', 'order', 'having', 'as', 'like', 'in', 'show', 'tables', 'describe'
    ];
    ...
    if (resultsSearch.trim() !== '') {
      ...
      results = results.filter(row => 
        Object.values(row).some(val => 
          val !== null && val !== undefined && ('' + val).toLowerCase().includes(query)
        )
      );
    }
  ...
  const sqlTemplates = [
    ...
    { title: 'Sample Student Listing', sql: 'SELECT * FROM studentinfo_omr LIMIT 50;' },
    { title: 'Count Students by Group', sql: 'SELECT group_name, COUNT(*) as count FROM studentinfo_omr GROUP BY group_name;' },
    { title: 'Count Students by Medium/Shift', sql: 'SELECT medium, shift, COUNT(*) as count FROM studentinfo_omr GROUP BY medium, shift;' },
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold tracking-tight text-white font-display flex items-center gap-2">
              SQL Console <span className="text-xs bg-emerald-500/20 text-emerald-400 px-2 py-0.5 rounded border border-emerald-500/30">Supabase Engine</span>
            </h1>
            <p className="text-xs text-slate-400">Execute RAW SQL statements safely</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <div className="relative group">
            <button className="text-xs bg-slate-900 hover:bg-slate-800 text-slate-300 px-3.5 py-2 rounded-lg border border-slate-850 transition flex items-center gap-1.5 font-medium">
              <Code className="w-3.5 h-3.5 text-emerald-400" /> Query Snippets <ChevronDown className="w-3 h-3 text-slate-500" />
            </button>
            <div className="absolute right-0 mt-2 w-64 bg-slate-900 border border-slate-850 rounded-xl shadow-2xl overflow-hidden opacity-0 scale-95 pointer-events-none group-hover:opacity-100 group-hover:scale-100 group-hover:pointer-events-auto transition-all duration-150 z-50">
              <div className="px-3.5 py-2 bg-slate-950/50 border-b border-slate-850 text-[10px] uppercase font-bold text-slate-400 tracking-wider">Common Templates</div>
      ...
      <div className="flex-1 flex flex-col lg:flex-row h-[calc(100vh-76px)] overflow-hidden">
        ...
        <aside className="w-full lg:w-72 bg-slate-950/60 border-r border-slate-900 flex flex-col shrink-0">
          <div className="px-5 py-3.5 bg-slate-950 border-b border-slate-900 flex items-center justify-between text-xs text-slate-450 uppercase font-bold tracking-wider">
            <span className="flex items-center gap-1.5"><Database className="w-3.5 h-3.5 text-emerald-400" /> Database Schema</span>
            <button 
              onClick={fetchTables} 
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-2">
            {schemaLoading && tables.length === 0 ? (
              ...
              <div className="space-y-2">
                {[...Array(4)].map((_, idx) => (
                  <div key={idx} className="h-9 rounded-lg bg-slate-900/50 border border-slate-850/40 skeleton-shimmer"></div>
                ))}
              </div>
            ) : schemaError ? (
              <div className="p-4 border border-dashed border-red-500/20 rounded-xl bg-red-950/10 text-center">
                <AlertTriangle className="w-6 h-6 text-red-400 mx-auto mb-2" />
                <p className="text-xs text-red-200 font-semibold mb-2">Error loading schema</p>
                <p className="text-[10px] text-red-405/85 mb-3 leading-normal">{schemaError}</p>
                <button 
                  onClick={fetchTables}
              ...
              <div className="space-y-1">
                {tables.map((t, idx) => (
                  <div key={idx} className="rounded-lg border border-slate-900/60 overflow-hidden bg-slate-900/10">
                    {/* Table row trigger */}
                        ...
                        {t.loading ? (
                          ...
                          <div className="grid grid-cols-1 gap-1">
                            {t.columns.map((col, cIdx) => (
                              <div key={cIdx} className="flex items-center justify-between font-mono text-[10px] text-slate-400 hover:text-slate-200 transition py-0.5">
                                <span className="truncate font-semibold flex items-center gap-1">
                                  <span className="text-emerald-500">{col.Key === 'PRI' ? '🔑' : '•'}</span>
                                  {col.Field}
                                </span>
                                <span className="text-slate-600 font-normal shrink-0 text-right">{col.Type}</span>
                              </div>
                            ))}
                          </div>
                        ) : (
                          <div className="text-slate-600 italic py-1">No columns discovered</div>
                        )}
                      </div>
                    )}
                  </div>
          ...
          <div className="p-4 bg-slate-950 border-t border-slate-900 text-[10px] text-slate-500 leading-normal flex items-start gap-2 shrink-0">
            ...
            <p>Write your raw SQL query on the editor. Press <kbd className="bg-slate-900 px-1 py-0.5 rounded border border-slate-800 text-[9px] font-mono text-slate-400">Ctrl+Enter</kbd> to run.</p>
          </div>
        </aside>
        ...
        <section className="flex-1 flex flex-col h-full bg-[#090e18] overflow-hidden">
          ...
          <div className="bg-slate-950 border-b border-slate-900 px-4 py-2 flex items-center justify-between shrink-0">
            <div className="flex items-center gap-1 overflow-x-auto scrollbar-none">
              {tabs.map(tab => (
                <button
                  ...
                  className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border text-xs font-semibold font-mono transition shrink-0 ${activeTabId === tab.id ? 'bg-slate-900 border-slate-850 text-emerald-400' : 'bg-transparent border-transparent hover:bg-slate-900/50 hover:border-slate-900 text-slate-400 hover:text-slate-200'}`}
                >
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              >
                <Play className="w-3.5 h-3.5 fill-white" /> {running ? 'Running...' : 'Execute'}
              </button>
            </div>
          </div>
          ...
          <div className="h-48 border-b border-slate-900 relative bg-[#0c1220] flex">
            {/* Pseudo line numbers */}
            <div className="select-none text-right pr-2 pl-3 py-3.5 bg-slate-950/20 text-slate-600 font-mono text-xs border-r border-slate-900 shrink-0 text-right min-w-[2.5rem]">
            ...
            <textarea
              ...
              className="flex-1 p-3.5 font-mono text-sm leading-5 bg-transparent border-0 focus:outline-none resize-none text-emerald-100 placeholder-slate-600 select-text outline-none focus:ring-0"
              spellCheck="false"
              autoFocus
            />
          </div>
          ...
          <div className="bg-slate-950 border-b border-slate-900 px-4 flex items-center justify-between shrink-0">
            <div className="flex items-center">
              ...
              >
                Console Logs {(sqlError) && <span className="w-1.5 h-1.5 rounded-full bg-red-500 inline-block ml-1.5" />}
              </button>
              <button
            ...
            {activePaneTab === 'results' && rows.length > 0 && (
              <div className="flex items-center gap-2 py-1.5">
                <span className="text-[11px] text-slate-500 font-mono font-medium mr-2">
          ...
          <div className="flex-1 overflow-auto bg-[#0a0f19] relative">
            ...
            {running && (
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-950/70 z-30 backdrop-blur-xs">
                ...
                <p className="text-sm font-semibold text-slate-300">Running database query...</p>
              </div>
            )}
              ...
              <div className="h-full w-full flex flex-col">
                {rows.length === 0 ? (
                  <div className="flex-1 flex flex-col items-center justify-center text-center p-8 text-slate-500">
                    ...
                    <p className="text-xs text-slate-650 mt-1 max-w-xs">Write a SELECT statement on the editor above and run to populate this view.</p>
                  </div>
                ) : (
                  <div className="flex-1 flex flex-col justify-between overflow-hidden">
                    ...
                    <div className="flex-1 overflow-auto custom-scrollbar">
                      <table className="w-full text-xs text-left border-collapse border-b border-slate-900 select-text">
                        <thead className="sticky top-0 bg-slate-950 border-b border-slate-900 text-slate-350 uppercase tracking-wider font-mono select-none">
                          <tr>
                            <th className="p-2 text-center border-r border-slate-900 w-12 bg-slate-950 z-20">SL</th>
                            {columns.map(col => (
                              <th
                                key={col}
                        ...
                        <tbody className="divide-y divide-slate-900 font-mono">
                          {paginatedRows.map((row, idx) => (
                            <tr key={idx} className="hover:bg-slate-900/40 bg-transparent transition">
                    ...
                    <div className="bg-slate-950 border-t border-slate-900 px-5 py-3 flex items-center justify-between shrink-0 text-xs select-none">
                      <div className="flex items-center gap-4">
                        <span className="text-slate-500 font-medium">
                          Showing {(currentPage - 1) * pageSize + 1} to {Math.min(currentPage * pageSize, totalItems)} of {totalItems} rows
                        </span>
                        <div className="flex items-center gap-1.5">
                          <span className="text-slate-650 text-[11px]">Rows per page:</span>
                          <select
                            value={pageSize}
                            onChange={(e) => {
            ...
            {activePaneTab === 'console' && (
              <div className="p-5 font-mono text-sm leading-6 space-y-4">
                {sqlError ? (
                  // DB Error Block
                  <div className="p-4 border border-dashed border-red-500/25 rounded-xl bg-red-950/15 flex gap-3 text-red-300">
                    <AlertTriangle className="w-5 h-5 shrink-0 text-red-400" />
                    <div>
                      <h4 className="font-bold text-red-200">Database Query Failed</h4>
                      <p className="text-xs text-red-400/90 mt-1 leading-normal">{sqlError}</p>
                    </div>
                  </div>
                ) : successInfo ? (
                  ...
                  <div className="p-4 border border-dashed border-emerald-500/25 rounded-xl bg-emerald-950/15 flex gap-3 text-emerald-300">
                    ...
                    <div>
                      <h4 className="font-bold text-emerald-250">Query Executed Successfully</h4>
                      <div className="grid grid-cols-2 gap-x-8 gap-y-1 text-xs mt-2 w-fit">
                        <span className="text-slate-500">Affected Rows:</span>
                        <span className="font-bold text-slate-200">{successInfo.affected_rows ?? 0}</span>
                        <span className="text-slate-500">Last Insert ID:</span>
                        <span className="font-bold text-slate-200">{successInfo.insert_id ?? 0}</span>
                        <span className="text-slate-500">Execution Time:</span>
                        <span className="font-bold text-slate-200">{execTime ? `${execTime} ms` : 'N/A'}</span>
                      </div>
                    </div>
                  </div>
                ) : rows.length > 0 ? (
                  ...
                  <div className="p-4 border border-dashed border-emerald-500/20 rounded-xl bg-emerald-950/10 flex gap-3 text-emerald-300">
                    <Check className="w-5 h-5 shrink-0 text-emerald-400" />
                    <div>
                      <h4 className="font-bold text-emerald-250">SELECT Statement Complete</h4>
                      <div className="grid grid-cols-2 gap-x-8 gap-y-1 text-xs mt-2 w-fit">
                        <span className="text-slate-500">Rows Returned:</span>
                        <span className="font-bold text-slate-200">{rows.length}</span>
                        <span className="text-slate-500">Execution Time:</span>
                        <span className="font-bold text-slate-200">{execTime ? `${execTime} ms` : 'N/A'}</span>
                      </div>
                    </div>
                  ...
                  <div className="text-slate-500 flex flex-col items-center justify-center py-12 text-center">
                    ...
                    <p className="text-xs">Database console is idle. Execute queries to see terminal logs.</p>
                  </div>
                )}
              </div>
            )}
                      ...
                      >
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-2 font-sans font-semibold">
                            <span className={`px-2 py-0.5 rounded text-[10px] uppercase ${item.status === 'success' ? 'bg-emerald-500/25 text-emerald-400' : 'bg-red-500/20 text-red-400'}`}>
                              {item.status}
                            </span>
                            <span className="text-slate-500">{item.timestamp}</span>
                            <span className="text-slate-600">•</span>
                            <span className="text-slate-500">{item.executionTimeMs} ms</span>
                          </div>
                          
                          <pre className="p-2.5 bg-slate-950 rounded border border-slate-850 font-mono text-xs text-slate-350 overflow-x-auto whitespace-pre-wrap max-h-20 max-w-full">
                            {item.sql}
                          </pre>
                          
                          <p className="text-[11px] text-slate-500 mt-1.5 leading-normal truncate">{item.message}</p>
                        </div>
