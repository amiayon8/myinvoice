...
export default function ICMRIExplorer() {
  ...
  const fetchDirectory = async (path: string, updateHistory = true) => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/icmri-proxy?path=${encodeURIComponent(normalized)}`,
      );

      if (!res.ok) {
        throw new Error(
          `Failed to load directory. Remote server returned ${res.status}`,
        );
      }
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        const res = await fetch(
          `/api/icmri-proxy?path=${encodeURIComponent(normalized)}&search=${encodeURIComponent(searchQuery.trim())}`,
        );
        if (res.ok) {
          const data: DirectoryResponse = await res.json();
          if (!data.error) {
  ...
  const handleSaveFile = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/icmri-proxy?path=${encodeURIComponent(normalizedPath)}`,
        {
          method: "POST",
  ...
  const previewFile = async (item: FileSystemItem) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(
          `Failed to load file contents. Status code ${res.status}`,
        );
      }
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans flex flex-col selection:bg-indigo-500/30 selection:text-indigo-200">
      ...
      <header className="border-b border-slate-800 bg-slate-950/60 backdrop-blur-xl sticky top-0 z-40 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">
              Secure Live Filesystem Manager
            </p>
          </div>
        </div>
        ...
        <div className="hidden md:flex items-center gap-2">
          ...
          <button
            onClick={() =>
              fetchDirectory("/home/icmriorg/public_html/RUMC/uploads_phd")
            }
            className="text-xs bg-slate-900 hover:bg-indigo-900/40 text-slate-300 hover:text-indigo-400 px-3 py-1.5 rounded-lg border border-slate-800 hover:border-indigo-500/30 transition duration-200 flex items-center gap-1.5"
          >
      ...
      <main className="flex-1 max-w-[1700px] mx-auto w-full p-4 lg:p-6 grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div
          className={`transition-all duration-300 ${selectedFile ? "lg:col-span-4" : "lg:col-span-12"} flex flex-col gap-4 w-full h-full`}
        >
          <div
            className={`${panelStyle} border border-slate-800/80 bg-slate-900/30 backdrop-blur-md flex flex-col h-[calc(100vh-280px)] min-h-[500px]`}
          >
            {/* Section Header */}
            <div className="px-5 py-3.5 bg-slate-950/80 border-b border-slate-800/80 flex items-center justify-between shrink-0">
              ...
              <span className="text-xs bg-slate-800 text-slate-450 px-2 py-0.5 rounded  shrink-0">
                {searchQuery.trim()
              ...
              {loading || searchLoading ? (
                ...
                <div className="space-y-2">
                  {[...Array(6)].map((_, index) => (
                    <div
                      key={index}
                      className="flex items-center gap-3 p-3 rounded-lg bg-slate-900/50 border border-slate-850/50 skeleton-shimmer"
                    >
                ...
                <div className="p-8 text-center border border-dashed border-red-500/20 rounded-xl bg-red-950/10">
                  ...
                  <p className="text-xs text-red-400/80 max-w-sm mx-auto mb-4">
                    {error}
                  </p>
                  <button
                    onClick={handleRefresh}
                ...
                <div className="text-center py-16 text-slate-500">
                  ...
                  {searchQuery && (
                    <p className="text-xs mt-1">
                      No items matched search "{searchQuery}"
                    </p>
                  )}
                </div>
              ) : viewMode === "list" ? (
                  ...
                  {filteredItems.map((item, idx) => (
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {searchQuery.trim() !== "" && (
                          <p className="text-[10px] text-indigo-400/80  truncate mt-0.5">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-2 xl:grid-cols-3 gap-3">
                  {filteredItems.map((item, idx) => (
                    ...
                    >
                      ...
                      <p className="text-xs font-semibold max-w-full truncate px-1 w-full">
                        {item.name}
                      </p>
                      {searchQuery.trim() !== "" && (
                        <p className="text-[9px] text-indigo-400/80  truncate mt-0.5 w-full text-center">
                          in {getItemFolderPath(item)}
                        </p>
                      )}
                      {item.type === "file" && (
        ...
        {selectedFile && (
          <div
            className={`transition-all duration-300 lg:col-span-8 flex flex-col gap-4 w-full h-full ${isFullscreen ? "fixed inset-0 z-50 p-4 bg-slate-950" : ""}`}
          >
            <div
              className={`${panelStyle} border border-slate-800 bg-slate-950 flex flex-col ${isFullscreen ? "h-full" : "h-[calc(100vh-280px)] min-h-[500px]"}`}
            >
              {/* File Viewer Header */}
                ...
                <div className="flex items-center gap-2.5 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[10px] text-slate-400  mt-0.5 truncate">
                      ...
                      {selectedFile.mime_type || "Unknown Type"} • Permissions:{" "}
                ...
                {fileLoading ? (
                  ...
                  <div className="w-full h-full overflow-hidden p-0 flex flex-col bg-[#0c1220] relative">
                    ...
                    <div className="flex-1 flex flex-row overflow-auto custom-scrollbar">
                      {/* Line numbers column */}
                      <div className="select-none text-right pr-3 pl-4 py-4 bg-slate-950/30 text-slate-650  text-sm border-r border-slate-900 shrink-0 min-w-[3.5rem] leading-6">
                        {editContent.split("\n").map((_, index) => (
                      ...
                      <textarea
                        ...
                        disabled={saveLoading}
                        className="flex-1 p-4  text-sm leading-6 bg-transparent border-0 focus:outline-none resize-none text-emerald-100 select-text outline-none focus:ring-0 min-h-full h-fit overflow-y-visible"
                        spellCheck="false"
                      />
                    </div>
                  </div>
                  ...
                  <div className="w-full h-full overflow-auto custom-scrollbar p-0 flex flex-row">
                    ...
                    <div className="select-none text-right pr-3 pl-4 py-4 bg-slate-950/30 text-slate-650  text-sm border-r border-slate-900 shrink-0 text-right min-w-[3.5rem]">
                      {fileContent.split("\n").map((_, index) => (
                    ...
                    <pre className="flex-1 m-0 p-4  text-sm leading-6 overflow-x-auto bg-transparent select-text">
                      <code
                        ref={codeRef}
                        className={`${getLanguageClass(selectedFile.name)} block`}
                      >
                        {fileContent}
                      </code>
                    </pre>
                    ...
                    selectedFile.name.endsWith(".webp") ? (
                      ...
                      <div className="text-center max-w-md">
                        ...
                        <p className="text-xs text-slate-400 mb-6 leading-relaxed">
                          This file type (
                          {selectedFile.mime_type || "binary/stream"}) is not
                          compatible with direct browser rendering or code
                          syntax highlights. You can download the file to
                          inspect it locally.
                        </p>
                        <a
                          href={`/api/icmri-proxy?path=${encodeURIComponent(normalizePath(selectedFile.relative_path))}`}
              ...
              <div className="px-5 py-3 bg-slate-900 border-t border-slate-800 shrink-0 text-xs text-slate-500 flex flex-col md:flex-row md:items-center justify-between gap-2">
                ...
                {selectedFile.timestamps.modified_at && (
                  <span className="shrink-0 flex items-center gap-1">
                    <Clock className="w-3.5 h-3.5 text-slate-650" /> Modified:{" "}
                    {selectedFile.timestamps.modified_at}
                  </span>
                )}
              </div>
      ...
      <footer className="border-t border-slate-900 bg-slate-950 py-4 px-6 text-center text-xs text-slate-600 shrink-0">
        <p>
          © 2026 ICMRI 2025. Powered by Next.js & React Server Components.
          Created with high performance proxy backend.
        </p>
      </footer>
    </div>
  );
